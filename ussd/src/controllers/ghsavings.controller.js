...
menu.state('pay',{
    run: () => {
        menu.con('Enter Daily Savings Start Amount. Minimum amount')
    },
    next: {
        '*\\d+': 'pay.option'
    }
...
menu.state('signup.successful',{
    run: () => {
        menu.end('Thank you for signing up for the GH Savings Challenge. Your details have been received. Your balance is Ghc 0')
    }
})
...
menu.state('pay.option', {
    run: () => {
        menu.con(`Authorize payment of GHS 3 to the GH Savings Challenge. \nEnter Momo Pin`)
    },
    next: {
        '*\\d+': 'pay'
    }
